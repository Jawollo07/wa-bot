...
function participantIdMatch(serialized, userId) {
    if (!serialized || !userId) return false;
    if (serialized === userId) return true;
    const a = normalizePhone(serialized);
    const b = normalizePhone(userId);
    if (a.length >= 8 && b.length >= 8 && (a === b || a.endsWith(b) || b.endsWith(a))) return true;
    return false;
}

function isParticipantAdmin(chat, userId) {
    if (!chat || !chat.participants || !userId) return false;
    const p = chat.participants.find(x => {
        const sid = x.id && (x.id._serialized || x.id);
        return participantIdMatch(sid, userId);
    });
...
async function handleViolation(msg, chat, groupId, senderId, reason, maxWarnings, isAdmin) {
    try {
        ...
        const number = senderId.split('@')[0].replace(/\D/g, '') || senderId.split('@')[0];
...
async function handleAdminCommands(msg, chat, settings, groupId) {
    ...
    if (command === p + 'stats') {
        ...
        const warnRow = warnRows[0], muteRow = muteRows[0], logRow = logRows[0];
    ...
    if (command === p + 'setwelcome') {
        ...
        await safeReply(msg, groupId, '✅ Willkommenstext gesetzt:\n' + text);
        return true;
    }
